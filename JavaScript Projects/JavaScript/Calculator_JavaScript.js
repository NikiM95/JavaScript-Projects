...
function Input_Digit(digit) {
    ...
    // We are cheaking to see if Wait_Second_Operand is true and set
...
function Handle_Operator(Next_Operator) {
    ...
    // Calculator.First_Operand if it does not already exist
    const Value_of_Input = parseFloat(Display_Value);
    // This checks if an operator already exists and if Wait_Second_Operand it true,
    ...
    if (First_Operand == null) {
        ...
        const Value_Now = First_Operand || 0 ; // press shift, fn and L to get "||"
        // If operator exists, property lookup is performed for the operator
        // in the Perform_Calculation object and the functionthat matches the
...
keys.addEventListener('click', (event) => {
    // The taget variable is an object that represents the element
    // that was clicked
    const { target } = event;
